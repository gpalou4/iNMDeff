...
QQplots_and_pval_histograms <- function(data,genelist,plots_type) {
            ...
            if (plots_type == "dataset") {
                plots_path <- paste0("/g/strcombio/fsupek_home/gpalou/analysis_results/NMD_project/associations/rare_germline_variants/TCGA/",genelist,"/",dataset_name,"/plots")
            } else if (plots_type == "all") {
                plots_path <- paste0("/g/strcombio/fsupek_home/gpalou/analysis_results/NMD_project/associations/rare_germline_variants/TCGA/",genelist,"/QC_plots")
                dataset_name <- "all"
            }
...
NMD_genes <- read.table( file = "/g/strcombio/fsupek_cancer1/gpalou/NMD_project/conversor_tables/ENSEMBL/NMD_genes.txt",
                        sep = "\t", header = TRUE)
datasets <- c("PTV_Missense_CADD15_0.1perc","PTV_Missense_CADD25_0.1perc" ,"MRT_25thCentile_0.1perc","CCR_90orhigher_0.1perc","PTV_0.1perc")
...
# 2) QQ-plots and lambdas
# For each NMD_method - tissue - dataset - Randomization vs non-randomization
all_lambdas <- TCGA_RGWAS_df_all %>%
        group_by(NMD_method,dataset,tissue) %>%
            do( QQplots_and_pval_histograms(.,"nogenelist","dataset") )
all_lambdas$database <- "TCGA"
# For each NMD_method - tissue - Randomization vs non-randomization
# TCGA_RGWAS_df_all %>%
#         group_by(NMD_method,tissue) %>%
#             do( QQplots_and_pval_histograms(.,"nogenelist","all") )
genelist <- "nogenelist"
output_path <- paste0("/g/strcombio/fsupek_home/gpalou/analysis_results/NMD_project/associations/rare_germline_variants/TCGA/",genelist,"/TCGA_",genelist,"_all_lambdas.txt")
