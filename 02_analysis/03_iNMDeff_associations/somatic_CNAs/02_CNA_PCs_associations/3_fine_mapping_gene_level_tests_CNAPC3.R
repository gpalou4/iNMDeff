...
# Candidate genes CNA-PC3 and CNA-PC86 --> Chr 1q amplification in q21-23.1
chr_char <- 1
...
# Save
write.table(CNA_RNAseq_iNMDeff, file = "/g/strcombio/fsupek_home/gpalou/Manuscript/figures_data/Fig3/Fig3D_2.txt", 
                sep = "\t", quote = FALSE, col.names = TRUE, row.names = TRUE)
saveRDS(CNA_RNAseq_iNMDeff, "/g/strcombio/fsupek_home/gpalou/Manuscript/figures_data/Fig3/Fig3D_2.RData")
write.table(CNA_RNAseq_iNMDeff, file = "/g/strcombio/fsupek_home/gpalou/Manuscript/figures_data/SuppFig15/SuppFig15A_2.txt", 
                sep = "\t", quote = FALSE, col.names = TRUE, row.names = TRUE)
saveRDS(CNA_RNAseq_iNMDeff, "/g/strcombio/fsupek_home/gpalou/Manuscript/figures_data/SuppFig15/SuppFig15A_2.RData")
...
# write.table(corr_RNAseq_iNMDeff, file = "/g/strcombio/fsupek_home/gpalou/Manuscript/figures_data/Fig3/Fig3D_2.txt", 
#                 sep = "\t", quote = FALSE, col.names = TRUE, row.names = TRUE)
# saveRDS(corr_RNAseq_iNMDeff, "/g/strcombio/fsupek_home/gpalou/Manuscript/figures_data/Fig3/Fig3D_2.RData")
...
# Bands & top genes
bands <- c("q21.1","q21.2","q21.3","q22","q23.1") # Chr1
...
corr_CNA_RNAseq_iNMDeff <- corr_CNA_RNAseq_iNMDeff %>%
        mutate(top_hits = if_else(CNA_RNA_corr >= 0.25 & ETG_iNMDeff_RNA_corr <= -0.15 & selected_bands != "other", gene_symbol,"NA"))
# Scatterplot
...
# Save
write.table(corr_CNA_RNAseq_iNMDeff, file = "/g/strcombio/fsupek_home/gpalou/Manuscript/figures_data/Fig3/Fig3D.txt", 
                sep = "\t", quote = FALSE, col.names = TRUE, row.names = TRUE)
saveRDS(corr_CNA_RNAseq_iNMDeff, "/g/strcombio/fsupek_home/gpalou/Manuscript/figures_data/Fig3/Fig3D.RData")
write.table(corr_CNA_RNAseq_iNMDeff, file = "/g/strcombio/fsupek_home/gpalou/Manuscript/figures_data/SuppFig15/SuppFig15A.txt", 
                sep = "\t", quote = FALSE, col.names = TRUE, row.names = TRUE)
saveRDS(corr_CNA_RNAseq_iNMDeff, "/g/strcombio/fsupek_home/gpalou/Manuscript/figures_data/SuppFig15/SuppFig15A.RData")
# corr_CNA_RNAseq_iNMDeff <- readRDS("/g/strcombio/fsupek_home/gpalou/Manuscript/figures_data/Fig3/Fig3D.RData")
...
candidate_genes <- c(candidate_genes,"SF3B4")
# candidate_genes <- c("CRTC1","NUP153","KDM6B","PXDN","LAMC1","COL19A1","TRAP1","PDIA2","FIG4","CCDC114")
candidate_genes <- candidate_genes[!candidate_genes %in% c("SMG5","SMG7","RBM8A")]
...
tmp_df <- corr_CNA_RNAseq_iNMDeff %>%
        ...
        filter(! band %in% c("q23.2","q23.3","q24.2","q24.3","q25.1","q25.2","q25.3","q31.1","q31.2","q31.3")) %>%
        filter(CNA_RNA_corr < 0.25 & ETG_iNMDeff_RNA_corr > -0.15 )
...
final_candidate_genes <- c(candidate_genes,NMD_genes$gene_symbol,control_genes)
...
# Save
write.table(df, file = "/g/strcombio/fsupek_home/gpalou/Manuscript/figures_data/Fig3/Fig3F.txt", 
                      sep = "\t", quote = FALSE, col.names = TRUE, row.names = TRUE)
saveRDS(df, "/g/strcombio/fsupek_home/gpalou/Manuscript/figures_data/Fig3/Fig3F.RData")
...
# 1q genes
genes_chr1q <- TCGA_CNV_genes_focal_filt$gene_name
# genes_chr1q <- c(genes_chr1q,c("CRTC1","NUP153","KDM6B","PXDN","LAMC1","COL19A1","TRAP1","PDIA2","FIG4","CCDC114")) # germline hits
genes_chr1q_candidate_region <- TCGA_CNV_genes_focal_filt[TCGA_CNV_genes_focal_filt$band %in% bands,"gene_name"]
# genes_chr1q_candidate_region <- c(c("CRTC1","NUP153","KDM6B","PXDN","LAMC1","COL19A1","TRAP1","PDIA2","FIG4","CCDC114")) # germline hits
# Filt
cols <- colnames(DepMap_snf_rpca) %in% c(NMD_factors,genes_chr1q)
rows <- rownames(DepMap_snf_rpca) %in% c(NMD_factors,genes_chr1q)
DepMap_snf_rpca_filt <- DepMap_snf_rpca[rows,cols]
...
DepMap_snf_rpca_filt$gene_type <- ifelse(DepMap_snf_rpca_filt$gene_type %in% genes_chr1q_candidate_region, "1q21.1-23.1", DepMap_snf_rpca_filt$gene_type)
DepMap_snf_rpca_filt$gene_type <- ifelse(!DepMap_snf_rpca_filt$gene_type %in% c("NMD_gene","1q21.1-23.1"), "1q_rest", DepMap_snf_rpca_filt$gene_type)
...
candidate_genes <- row.names(DepMap_snf_rpca_filt[DepMap_snf_rpca_filt$gene_type == "1q21.1-23.1" | rownames(DepMap_snf_rpca_filt) %in% c("INTS3","SMG5","SMG7","RBM8A"),])
# candidate_genes <- row.names(DepMap_snf_rpca_filt[DepMap_snf_rpca_filt$gene_type == "1q21.1-23.1",])
cols <- colnames(DepMap_snf_rpca_filt) %in% c(candidate_genes,"gene_type")
rows <- DepMap_snf_rpca_filt$gene_type != "1q21.1-23.1"
...
# Obtain a p-value from a wilcox-test between mean of RPCO scores of NMD_gene vs 1q_rest for each 1q gene
# Test is not two-sided, we only want to test that the means between 1q_rest and NMD_gene is negative ("less")
all_final_df <- c()
...
for (gene in na.omit(unique(DepMap_snf_rpca_filt_stacked$candidate_gene))) {
        ...
        X <- df[df$gene_type == "1q_rest","correlations"]
        ...
        # Test means that X (1q_rest) group are sistematically producing lower values than Y (NMD_gene)
...
all_final_df$gene_type <- ifelse(all_final_df$gene %in% c(NMD_factors,"RBM8A","INTS3","SF3B4"), "NMD_related", "chr1q")
...
plot_final <- ggplot(data = all_final_df, aes(x = as.numeric(as.character(genome_location_chr)), y = -log10(p_value_FDR_adjust), #fill = as.numeric(diff),
        ...
        theme_classic()+
        labs(title = "", x = "Chr 1q 21.1-23.1", y = "FDR adjusted p-value") +
        geom_hline(yintercept= -log10(0.05), size = 0.75, color = "black")
output_path <- paste0("/g/strcombio/fsupek_home/gpalou/analysis_results/NMD_project/associations/CNV_associations/fine_mapping/selected_genes/Manhattan_plot_chr1q_RPCO_CoDep_p_values_CNA_PC_",CNA_PC,".png")
png(output_path, width = 3000, height = 3000, res = 300)
print(plot_final)
dev.off()

# Save
write.table(all_final_df, file = "/g/strcombio/fsupek_home/gpalou/Manuscript/figures_data/SuppFig15/SuppFig15B.txt", 
                sep = "\t", quote = FALSE, col.names = TRUE, row.names = FALSE)
saveRDS(all_final_df, "/g/strcombio/fsupek_home/gpalou/Manuscript/figures_data/SuppFig15/SuppFig15B.RData")
...
ETG_iNMDeff_somatic_associations <- ETG_iNMDeff_somatic_associations[ETG_iNMDeff_somatic_associations$Gene_symbol %in% genes_chr1q,]
...
plot_final <- ggplot(data = ETG_iNMDeff_somatic_associations, aes(x = as.numeric(as.character(genome_location_chr)), y = -log10(som_CNV_amp_pval_FDR_adjust), #fill = as.numeric(diff),
        ...
        geom_hline(yintercept= -log10(0.05), size = 0.75, color = "black")
output_path <- "/g/strcombio/fsupek_home/gpalou/analysis_results/NMD_project/associations/CNV_associations/fine_mapping/selected_genes/Manhattan_plot_chr1q_CNA_vs_iNMDeff_p_values.png"
png(output_path, width = 3000, height = 3000, res = 300)
print(plot_final)
dev.off()
