...
# 1) Open NMDeff table
type <- "germline"
input_path <- paste0("/g/strcombio/fsupek_cancer1/gpalou/GTEx/NMDeff/NMDeff_datasets/",type,"_PTCs_all_GTEx.txt")
PTC_transcripts_all_GTEx <- read.table(file = input_path, header = TRUE, sep = "\t", stringsAsFactors =FALSE)
# 2) Filterings
median(PTC_transcripts_all_GTEx$NMD_efficiency_TPM,na.rm = TRUE)
# 2.1) Remove NAs in NMDeff
dim(PTC_transcripts_all_GTEx)
PTC_transcripts_all_GTEx$NMD_efficiency_TPM <- ifelse(PTC_transcripts_all_GTEx$NMD_efficiency_TPM == "Inf",NA,PTC_transcripts_all_GTEx$NMD_efficiency_TPM)
PTC_transcripts_all_GTEx$NMD_efficiency_TPM <- ifelse(PTC_transcripts_all_GTEx$NMD_efficiency_TPM == "-Inf",NA,PTC_transcripts_all_GTEx$NMD_efficiency_TPM)
...
NMDeff_PTCs_transcript <- aggregate(NMD_efficiency_TPM ~ X55_nt_last_exon + transcript_id, data = PTC_transcripts_all_GTEx_filt, median)
...
# Save
output_path <- "/g/strcombio/fsupek_home/gpalou/data/NMD_project/conversor_tables/ENSEMBL/GTEx_PTC_transcripts_NMDeff_diff.txt"
