...
# Modified function from ICAMS package
GetStrelkaVAF_inhouse <- function(vcf, name.of.VCF = NULL) {

    stopifnot("data.frame" %in% class(vcf))
    if (!("TUMOR" %in% names(vcf)) || !("FORMAT" %in% names(vcf))) {
        stop("\nVCF ", ifelse(is.null(name.of.VCF), "", paste0(dQuote(name.of.VCF), 
            " ")), "does not appear to be a Strelka VCF, column names are \n", 
            paste(colnames(vcf), collapse = " "))
    }
    # Split in snvs and indels
    #indels_index <- grep("frameshift",vcf$ExonicFunc.refGene)
    indels_index <- grep("TAR:TIR",vcf$FORMAT)
    if (length(indels_index) == 0) {
      vcf_snvs <- vcf
      muts <- c("snv")
    } else {
      vcf_snvs <- vcf[-indels_index,]
      vcf_indels <- vcf[indels_index,]
      muts <- c("snv","indel")
    }
    ...
    for ( mut_type in muts ) {
      if (mut_type == "snv") {
        ...
        each.base.col <- c("TAR","TIR")
        alt_char <- "TIR"
      }
      TUMOR <- as.character(vcf_tmp[["TUMOR"]])
      control <- as.character(unique(vcf_tmp[["FORMAT"]]))
      alt <- vcf_tmp[["ALT"]]
      stopifnot(length(control) == 1)
      colnames <- unlist(strsplit(control, split = ":", fixed = TRUE))
      values <- strsplit(TUMOR, split = ":", fixed = TRUE)
      vaf <- numeric(nrow(vcf_tmp))
      read.depth <- integer(nrow(vcf_tmp))
      ...
      for (i in 1:length(vaf)) {
          ...
          if (mut_type == "snv") {
            alt_char <- paste0(alt[i], "U")
          }
          alt.count <- tier1.counts[alt_char]
          vaf[i] <- alt.count/total.read.count
          read.depth[i] <- total.read.count
      }
      vcf_tmp <- cbind(vcf_tmp, VAF = vaf, read.depth = read.depth)
      if (length(vcf_final) == 0) {
        vcf_final <- vcf_tmp
      } else {
        vcf_final <- rbind(vcf_final,vcf_tmp)
      }
    }
    return(vcf_final)
}
...
VCF_extract_info <- function(VCF, VCF.type) {
    if (VCF.type == "somatic") {
      VCF.tmp <- VCF
      genotype.col <- "Otherinfo14"
      colnames(VCF.tmp)[colnames(VCF.tmp) %in% c("Alt","Otherinfo12",genotype.col)] <- c("ALT","FORMAT","TUMOR")
      # Get VAF for SNVs
      #VCF.tmp <- GetStrelkaVAF(VCF.tmp, name.of.VCF = NULL)
      VCF.tmp <- GetStrelkaVAF_inhouse(vcf = VCF.tmp, name.of.VCF = NULL)
      # Get VAF for Indels
    } else {
      genotype.col <- "Otherinfo13"
    }
    variants.list <- list()
    for (i in 1:nrow(VCF)) {
      ...
      transcripts.exons.info$genotype <- unlist(strsplit(as.character(VCF[i,genotype.col]),":"))[1]
...
NMD_rules <- function(VCF, VCF.type) {
  ...
  PTCs.list.NMD <- lapply(PTCs.list, function(PTC.variants) {
    ...
    for (i in 1:nrow(PTC.variants)) {
        ...
        # Add PTC_CDS_pos for missense/syn variants
        if (PTC$variant_type %in% c("nonsynonymous_SNV","synonymous_SNV")) {
          PTC[,"PTC_CDS_pos"] <- variant.cds.position
          PTC[,"stopgain"] <- PTC[,"variant_type"]
        }
...
VCF_sample_file <- function(TCGA.cancer, TCGA.barcode, type) {
  if (type == "germline") {
    VCF.path <- gsub("\\[X\\]",gsub("\\.","-", strsplit(TCGA.cancer,"-")[[1]][2]), VCF.germline.path)
    tmp.paths <- list.dirs(path = paste0(VCF.path,gsub("\\.","-",TCGA.barcode)), full.names = TRUE, recursive = TRUE)
    VCF.path.sample.germline <- paste0(tmp.paths[5],"/",strsplit(TCGA.cancer,"-")[[1]][2],"_annotated.HG_multianno.txt.gz")
    if (file.exists(VCF.path.sample.germline)) {
      print(VCF.path.sample.germline)
      VCF.sample <- read.table(file = VCF.path.sample.germline, header = TRUE, sep = "\t")
    } else {
      return(NULL)
    }
  } else if (type == "somatic") {
    VCF.path <- gsub("\\[X\\]",gsub("\\.","-", strsplit(TCGA.cancer,"-")[[1]][2]), VCF.somatic.path)
    tmp.paths <- list.dirs(path = paste0(VCF.path,gsub("\\.","-",TCGA.barcode)), full.names = TRUE, recursive = TRUE)
    VCF.path.sample.snvs <- paste0(tmp.paths[4],"/",TCGA.barcode,"_snvs_annotated.HG_multianno.txt.gz")
    VCF.path.sample.indels <- paste0(tmp.paths[4],"/",TCGA.barcode,"_indels_annotated.HG_multianno.txt.gz")
    existsSNVsFile <- file.exists(VCF.path.sample.snvs)
    existsIndelsFile <- file.exists(VCF.path.sample.indels)
    VCF.sample.snvs <- NULL
    VCF.sample.indels <- NULL
    if (existsSNVsFile) {
      print(VCF.path.sample.snvs)
      VCF.sample.snvs <- read.table(file = VCF.path.sample.snvs, header = TRUE, sep = "\t")
    } 
    if (existsIndelsFile) {
      print(VCF.path.sample.indels)
      VCF.sample.indels <- read.table(file = VCF.path.sample.indels, header = TRUE, sep = "\t")
      indels_number <- nrow(VCF.sample.indels)
    } 
    if ( !is.null(VCF.sample.snvs) & !is.null(VCF.sample.indels) ) {
      VCF.sample <- rbind(VCF.sample.snvs,VCF.sample.indels)
    } else if ( is.null(VCF.sample.snvs) & !is.null(VCF.sample.indels) ) {
      VCF.sample <- VCF.sample.indels 
    } else if ( !is.null(VCF.sample.snvs) & is.null(VCF.sample.indels) ) {
      VCF.sample <- VCF.sample.snvs
    } else {
      return(NULL)
    }
  }
...
TCGA.cancer <- args[2] # e.g. TCGA-UCEC
TCGA.barcode <- args[3] # e.g. TCGA-FI-A2D6
VCF.type <- args[4] # somatic // germline
variant.type <- args[5] # stopgain // missense
print(args)
...
# TCGA.cancer <- "TCGA-UCS"
# TCGA.barcode <- "TCGA-QN-A5NN"
# VCF.type <- "somatic"
...
VCF.somatic.path <- paths[paths$folder_or_object=="VCF_somatic_file_path","path_or_filename"]
transcripts.fasta.path <- paths[paths$folder_or_object=="transcripts_fasta_path","path_or_filename"]
kozak.pwm.path <- paths[paths$folder_or_object=="kozak_pwm_path","path_or_filename"]
PTC.transcripts.output.path <- paths[paths$folder_or_object=="PTC_transcripts_output_path","path_or_filename"]

# Output path
output.path <- gsub("\\[X1\\]",TCGA.cancer, PTC.transcripts.output.path)
output.path <- gsub("\\[X2\\]",gsub("\\.","-",TCGA.barcode), output.path)
dir.create(output.path, showWarnings = FALSE, recursive = TRUE)
output.path <- paste0(output.path,paths[paths$folder_or_object==paste0(VCF.type,"_PTC_transcripts_output"),"path_or_filename"])
if (variant.type == "missense") {
  output.path <- gsub(paste0(VCF.type,"_PTC_"),paste0(VCF.type,"_missense_syn_"),output.path)
}
print(output.path)
...
if (!file.exists(output.path)) {
  ...
  # 2) NMD rules
  print(paste0("Starting NMD rules search for TCGA sample --> ",TCGA.barcode))
  print(paste0("PTCs from ",VCF.type," variants"))
  # 2.1) VCF data
  # Obtain VCF germline data file
  # Check if file exists
  VCF.sample <- NULL
  VCF.sample <- VCF_sample_file(TCGA.barcode = TCGA.barcode, TCGA.cancer = TCGA.cancer, type = VCF.type)
...
########## TO DO #############
# cambIar el output y añadir un parametro para lanzarlo en el cluster